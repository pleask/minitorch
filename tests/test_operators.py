...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert(sigmoid(0.5) == 0) # TODO: will it get this right or do we need to check whether it's within EPS?
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    o = sorted([a, b, c])
    assert lt(o[0], o[1]) and lt(o[1], o[2]) and lt(o[0], o[2])
...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    assert mul(z, add(x, y)) == add(mul(x, z), mull(y, z))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert sum(ls1) + sum(ls2) == sum(ls1 + ls2)
